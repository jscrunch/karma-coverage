...
var CoverageReporter = function(rootConfig, emitter, helper, logger) {
  ...
  this.onBrowserComplete = function(browser, result) {
    ...
    helper.mkdirIfNotExists(path.resolve(outDir), function() {
      ...
      var coverageResult = collector.getFinalCoverage();
      var completeResults = {
          results: testResults[browser.id],
          files: coverageResult
        };

      fs.writeFile(path.join(outDir, name), JSON.stringify(completeResults), 'utf8', function(err) {
        if (err) {
          log.error(err);
        }
        writeEnd();
  ...
  this.onSpecComplete = function(browser, result) {
    ...
    if (result.coverage) {
      console.log('spec complete with coverage');
      collectors[browser.id].add(result.coverage);
    }

    // Store the test result
    testName = [result.suite.join(' '), result.description].join(' ');
    testResults[browser.id][testName] = result.success ? 1 : 0;
  };

  this.onRunComplete = function(browsers, results) {
    console.log('Run Complete 3');
